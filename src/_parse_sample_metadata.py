...
clinical_vars = (
    ["disease", "organ", "phenotypes", "classification", "cause_of_death_text"]
...
for col in pathology_vars:
    meta["pathology:" + col + "_clean"] = (
        ...
        .replace({k: True for k in true_values + ["FOCAL", "focal", "BIZARRE"]})
...
lesions.columns = [x.replace(" ", "_") for x in "lesions:" + lesions.columns]
...
meta["smoker"] = pd.Categorical(
    meta["smoker"], categories=["No", "Former", "Yes"], ordered=True
)
